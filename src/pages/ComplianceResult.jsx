import { AlertCircle, CheckCircle2, FileDown, Lightbulb, Wrench } from "lucide-react";
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-1 flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 className="text-2xl font-semibold">{lastResult.productName}</h2>
            <p className="mt-1 text-sm text-muted">{lastResult.sku} · {lastResult.manufacturer}</p>
          </div>
          <div className="rounded-2xl bg-gradient-to-br from-brand-soft to-white px-5 py-4 text-right">
            <p className="text-xs uppercase tracking-wide text-muted">Overall score</p>
            <p className="text-3xl font-semibold text-brand">{lastResult.score}%</p>
            <p className="text-sm font-medium text-amber-700">{lastResult.status}</p>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <ul className="mt-4 space-y-3">
          {lastResult.checks.map((check) => (
            <li key={check.id} className="flex gap-3 rounded-xl border border-slate-100 p-4">
              {check.status === "pass" ? (
      ...
      <p className="rounded-xl bg-slate-100 px-4 py-3 text-xs text-slate-600">
        Demo assessment only. Final legal compliance should be verified against applicable regulations.
      </p>
    </div>
  );
}
